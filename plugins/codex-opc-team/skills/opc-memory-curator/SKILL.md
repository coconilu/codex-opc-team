...
1. Resolve the skill directory as the directory containing this `SKILL.md`, then resolve the plugin root as `<skill-dir>/../..`; read `references/promotion-policy.md` and `references/python-runtime.md`.
...
4. Verify source, evidence, scope, owner, confidence, duplicates, contradictions, and supersession links.
...
6. Replay the lesson on historical or synthetic cases, or run it in shadow mode without changing authority.
...
8. Apply the approved canonical transition: approve with `<memory-python> "<plugin-root>/scripts/opc_memory.py" approve <id> --approved-by <manager> --validation <evidence>`; reject with `<memory-python> "<plugin-root>/scripts/opc_memory.py" reject <id> --rejected-by <manager> --reason <reason>`; or mark approved guidance obsolete with `<memory-python> "<plugin-root>/scripts/opc_memory.py" obsolete <id> --reason <reason> [--superseded-by <new-id>]`. Create a new candidate instead of silently narrowing an existing record. `approve` never writes to the provider index; when Mem0 is enabled, require `_recall_sync=pending_commit` and treat it as an instruction to commit, not as indexing success.
9. Set `<transition-paths>` to only that ID's old and new canonical files: `candidates -> approved`, `candidates -> rejected`, or `approved -> obsolete`. From `knowledge_root`, show `git status --short -- <transition-paths>` and `git diff -- <transition-paths>`. Set `<stage-paths>` to the new path plus the old path only when `git ls-files --error-unmatch -- <old-path>` shows Git tracked it; an uncommitted candidate may have no trackable old deletion. Run `git add -A -- <stage-paths>` and show `git diff --cached -- <transition-paths>`. Quote each path separately; use no directory or wildcard pathspecs, and never stage unrelated user changes.
