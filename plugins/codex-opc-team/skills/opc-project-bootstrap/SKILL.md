...
6. Ensure the repository `.gitignore` contains the exact lines from `assets/gitignore.snippet`: `/.opc/run.json`, `/.opc/events.jsonl`, `/.opc/events.jsonl.*`, `/.opc/.opc-hook.lock`, and the private transaction boundary `/.opc/evolution/`. Do not ignore the whole `.opc` directory or overwrite broader ignore rules.
