...
Apply serializes work with a private data-root lock, revalidates the unchanged roots, exact source/destination, `lstat` object identity, regular-file type, Git tracked state, and absent destination, then binds the verified destination parent directory object. It atomically creates a same-filesystem hard link without overwrite and removes the source path only after proving the parent is still the same contained object and both paths still identify the approved file. A parent rename/symlink race rolls back only the newly linked object before preserving the source. A cross-filesystem or race failure requires a separately reviewed manual move. No command in this workflow deletes unrelated data, commits, uploads, or edits event contents.
