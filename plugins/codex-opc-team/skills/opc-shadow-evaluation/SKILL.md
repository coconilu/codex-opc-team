...
6. Present the preview fingerprint. Continue only after the user confirms that exact preview and a private artifact root outside the plugin, canonical knowledge, and project source.
...
8. Use `report` to render an existing machine result without mutation. Preserve measured, human judgment, model inference, and unverified evidence as separate classes.
