#!/usr/bin/env python3
"""Shared OPC snapshot, redaction, DTO, and local read-service core.

The service is deliberately a projection over existing File/Git and project
...
class SnapshotError(RuntimeError):
    """A safe snapshot error whose code is suitable for redacted output."""
...
# Preserve the old public/import contract while adapters migrate to SnapshotError.
DashboardError = SnapshotError
...
class SnapshotService:
    """One semantic source for Dashboard and OPC App snapshot projections."""

    def __init__(
        self,
        *,
        project_roots_provider: Callable[[], Sequence[Path | str]],
        knowledge_root: Path | str | None = None,
        data_root: Path | str | None = None,
        demo: bool = False,
        allow_empty: bool = False,
        demo_asset_root: Path | str | None = None,
    ...
    def snapshot(self) -> dict[str, Any]:
        if self._demo:
            snapshot = load_demo_snapshot(self._demo_asset_root)
        else:
            if self._knowledge_root is None or self._data_root is None:
                raise SnapshotError("LIVE_ROOTS_REQUIRED")
            ...
            if self._now_provider is not None:
                arguments["now"] = self._now_provider
