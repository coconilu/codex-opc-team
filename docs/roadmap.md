...
v0.2 release-level contract、公开 synthetic runner 和严格私有 aggregate/envelope Gate 已实现，当前状态明确为 `BLOCKED`。退出 R4 还必须完成[代表性私有 3–5 task pilot 与 exact release commit 验证](release-readiness-v0.2.0.md)；公开 fixture、模板、Developer 自测或单项 token/latency 改善都不能替代。`main` 已提供兼容的本地只读 Dashboard，以及可独立安装、只写 App 自有项目清单的本地 OPC App；二者共用固定 snapshot 语义。治理写操作、知识导出、合并、远程访问和宿主 Adapters 仍是后续候选，不属于 v0.2 发布阻断项。
