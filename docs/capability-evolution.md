...
- Every source candidate, feedback, evaluation, lineage, manager, QA, Shadow, observation, and rollback reference resolves to `capability-evolution-evidence.v1.schema.json`. Every envelope binds the canonical `proposal_core_sha256`: the complete governance proposal (project, scope, owner, pilot bounds and allowed projects, target, current/candidate/rollback provenance, versions, and creation time) excluding only circular evidence refs. Reusing a proposal id after any governance change fails closed.
- Evaluation creates an ordered full-arm `pilot_snapshot_sha256` covering case/run ids, capability and knowledge versions, lineage refs, status/reason, measurements or null, and confounders. The evaluation result is deterministically recomputed from that snapshot and has its own digest. Evaluation and promotion manager/QA/Shadow envelopes must bind all three digests; record reads, reports, promotion, and confirmation recompute them before use.
...
- Git objects must be regular `blob` objects in mode `100644` or `100755`. Candidate and confirmation ranges must be strict linear descendants: merge commits, symlinks, gitlinks, trees, type changes, renames/copies, empty commits, source-history resets, and any intermediate non-target path are rejected. Every intermediate target blob is read directly from Git object storage and independently checked for size, UTF-8, credentials, private markers, runtime ids, and host paths; a later clean blob cannot hide sensitive history.
- Private evidence reads bind the evidence parent directory, open the child without following links, require one regular filesystem link, read bounded bytes from one descriptor, and verify descriptor/path/parent identity again afterward. Symlink, junction/reparse, hardlink, same-size replacement, or parent-rename races fail closed without echoing evidence bodies.
