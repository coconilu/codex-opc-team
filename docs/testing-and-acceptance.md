...
| 维度 | 通过条件 |
|---|---|
| 最短路径 | 一条命令启动，打印可打开的本地 URL，`Ctrl+C` 可停止 |
| 数据 | demo、真实项目、缺失可选源和损坏输入均产生固定 Schema 的安全结果 |
| 隐私 | 响应不包含绝对路径、正文、凭据、会话标识或原始记录 |
| 网络 | 仅 loopback；错误 Host、跨源 Origin、非 `GET`/`HEAD` 与未知路由 fail closed |
| 语义 | File/Git 保持权威；Mem0 disabled 非故障；链路固定为 association/evidence only |
