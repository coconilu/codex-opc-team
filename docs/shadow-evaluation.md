...
| `evaluation/schemas/shadow-result.v1.schema.json` | 机器报告顶层契约 |
...
Artifact root 会被父目录对象绑定；已存在同名结果、symlink/reparse、hard link、父目录 identity 变化、大小越界或凭据模式都会失败关闭。错误只返回类别，不回显匹配的敏感值。
