...
| capability candidate 绕过经理/QA 或夹带无关 Git 改动 | exact blob/HEAD、同合同 paired pilot、两阶段经理与独立 QA 门禁、clean tree、单个 allowlisted unstaged diff、显式 commit/confirm；禁止全局配置写入 |
...
| Capability evolution sidecar | exact proposal、批准、paired pilot、evaluation、transition 与 observation | 项目私有审计策略；与 active capability 分离 | 可删除派生 sidecar；不得借此删除 Git 历史或 approved knowledge |
