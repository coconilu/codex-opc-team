...
若 measured 质量/安全比较同时包含 supporting 与 counterevidence，结果必须保持 `inconclusive/inconclusive`，并且恰好保留一个 `conflicting_measured_results` failure；归档或迁移不能通过清空 failure、改成 conclusive 或伪造 positive 来改变该事实。仅含 counterevidence 的 harmful 结果仍是 conclusive/do-not-promote，不属于冲突。
