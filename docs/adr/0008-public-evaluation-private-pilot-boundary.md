...
真实项目试点固定为 3–5 tasks。逐任务原始证据始终留在获批的私有项目边界，不进入公开仓库、canonical knowledge 或召回索引；公开或跨边界输出只允许严格 schema 的整体计数与分布，不允许项目名、自由文本、路径、逐任务数组和 artifact 引用。scope leakage 与 stale/obsolete acceptance 是零容忍硬门禁；缺字段、零分母和无法验证不能解释为 PASS。
