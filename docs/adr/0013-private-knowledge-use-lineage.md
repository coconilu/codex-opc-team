...
写入必须先零写入 preview，把 base sidecar 的存在性与原始文件字节 SHA-256 纳入 exact plan token，再用 base-record CAS、revision CAS、独占 lock、单链接文件和已审计的 `BoundDirectory` 事务原子发布。Git worktree 与 ignore 检测失败不得按 non-Git 放行。同 event ID 同内容为幂等 no-op；同 ID 不同内容、同 revision 替换、首次创建竞态、stale revision、并发冲突和父目录身份变化均 fail closed。错误不回显输入正文。v0.1 run 没有 sidecar 时返回 `lineage unavailable`，不迁移 `run.json`，也不伪造默认状态。
